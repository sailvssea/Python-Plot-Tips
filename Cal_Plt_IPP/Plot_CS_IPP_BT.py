...
# 'MST2_SEC_IR3_MSS_20120826_1232.csv'
pd_BTdata = pd.read_csv('MST2_SEC_IR3_MSS_20120820_1232.csv',header = None)
np_BTdata = np.flip((np.array(pd_BTdata)/100.0-273.15).transpose(),1)
# configure the figure/plot setting/environment
# Create a fixed sized figure (width, height in inches)
fig = plt.figure(figsize=(8.5,6.0))  # create the figure with the fix size
fig.subplots_adjust(left= 0.03, right= 0.95,\
					bottom= 0.05, top= 0.95, hspace= 0.20)

# %% GPS L1
ax1 = fig.add_subplot(221)
# setup Lambert Conformal basemap.
m = Basemap(llcrnrlon=100.,llcrnrlat=5.,urcrnrlon=135.,urcrnrlat=32,\
			 projection='merc',resolution='l',lat_ts=20.)
...
col,row = np.meshgrid(np.linspace(0.04,49,512),np.linspace(99.75,150.23,512))
cmap = plt.get_cmap('jet_r')
norm = matplotlib.colors.Normalize(vmin = -90, vmax = -10)
m.pcolormesh(row,col,np_BTdata,cmap= cmap, norm= norm, latlon = True)
#plt.title(i[17:-4]+' UTC')
# create bar
m.colorbar(location= 'right',label = 'Brightness Temperature (Celsius)')
...
for i in np.arange(32):
	...
	m.scatter(lonIPP_cs,latIPP_cs, s = 40, c = 'r', marker = '^', alpha = 1, zorder = 2,latlon = True)
	m.scatter(lonIPP_no_cs,latIPP_no_cs, s = 5, c = 'b', marker = '.', alpha = 0.5, zorder = 1,latlon = True)

plt.text(0.20, 0.05,'(a) GPS L1', fontsize = 12, fontweight = 'bold', ha = 'center', va = 'center', transform = ax1.transAxes)
# %%
...
m.scatter(lon_TC[11],lat_TC[11],marker= 'o',s= 300,c= 'c',alpha = 0.5,zorder=3,latlon=True)
...
plt.text(0.20, 0.05,'(b) GPS L2', fontsize = 12, fontweight = 'bold', ha = 'center', va = 'center', transform = ax2.transAxes)
# Plot the TC trajectory
latlon_TC = pd.read_csv('TEMBIN.TC',usecols=['Time','Latitude','Longitude'])
lon_TC = np.array(latlon_TC['Longitude'])
lat_TC = np.array(latlon_TC['Latitude'])
...
plt.text(0.20, 0.05,'(c) GLO L1', fontsize = 12, fontweight = 'bold', ha = 'center', va = 'center', transform = ax3.transAxes)
# Plot the TC trajectory
latlon_TC = pd.read_csv('TEMBIN.TC',usecols=['Time','Latitude','Longitude'])
lon_TC = np.array(latlon_TC['Longitude'])
lat_TC = np.array(latlon_TC['Latitude'])
...
plt.text(0.20, 0.05,'(d) GLO L2', fontsize = 12, fontweight = 'bold', ha = 'center', va = 'center', transform = ax4.transAxes)
# Plot the TC trajectory
latlon_TC = pd.read_csv('TEMBIN.TC',usecols=['Time','Latitude','Longitude'])
lon_TC = np.array(latlon_TC['Longitude'])
lat_TC = np.array(latlon_TC['Latitude'])
